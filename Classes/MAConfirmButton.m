...
@synthesize title, confirm, disabled, tint, toggleAnimation, confirmedTintColour;
...
- (void)toggle{    
    if(self.userInteractionEnabled){
		...
		if (!self.confirmedTintColour)
		{
			self.confirmedTintColour = [UIColor colorWithRed:0.439 green:0.741 blue:0.314 alpha:1.0];
		}
        ...
        if(disabled){
        colorAnimation.fromValue = (id)self.confirmedTintColour.CGColor;
        colorAnimation.toValue = (id)[UIColor colorWithWhite:0.85 alpha:1].CGColor;
        }else{
        colorAnimation.fromValue = selected ? (id)tint.CGColor : (id)self.confirmedTintColour.CGColor;
        colorAnimation.toValue = selected ? (id)self.confirmedTintColour.CGColor : (id)tint.CGColor;	
        }
