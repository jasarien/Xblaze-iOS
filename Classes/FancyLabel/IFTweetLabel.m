...
- (void)handleButton:(id)sender
{
	...
	//DebugLog(@"IFTweetLabel: handleButton: sender = %@, title = %@", sender, buttonTitle);
...
{
	//DebugLog(@"output = '%@', point = %@", text, NSStringFromCGPoint(point));
...
- (void)createButtons
{
	...
	while (NSMaxRange(scanRange) < textLength)
	{
		...
		{
			//DebugLog(@"------ whitespace: token = '%@'", token);
		...
		else
		{
			...
			if ([token isEqualToString:@"\r"] || [token isEqualToString:@"\n"])
			{
				...
				//DebugLog(@"------ scanText = '%@', token = '%@'", scanText, token);
				breakLine = YES;
			}
			BOOL breakWidth = NO;
			if (currentSize.width > frame.size.width)
			{
				...
				//DebugLog(@"------ scanText = '%@', currentSize = %@", scanText, NSStringFromCGSize(currentSize));
				breakWidth = YES;
			}
